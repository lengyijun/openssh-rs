...
extern "C" {
    ...
    fn sshbuf_dtourlb64(d: *const sshbuf, b64: *mut sshbuf, wrap: libc::c_int) -> libc::c_int;
    fn sshbuf_cmp(
        b: *const sshbuf,
        offset: size_t,
        s: *const libc::c_void,
...
unsafe extern "C" fn ssh_ecdsa_sk_cleanup(mut k: *mut sshkey) {
    sshkey_sk_cleanup(k);
    (sshkey_ecdsa_funcs.cleanup).expect("non-null function pointer")(k);
}
unsafe extern "C" fn ssh_ecdsa_sk_equal(mut a: *const sshkey, mut b: *const sshkey) -> libc::c_int {
...
unsafe extern "C" fn webauthn_check_prepare_hash(
    ...
    mut wrapper: *const sshbuf,
    mut flags: uint8_t,
    mut extensions: *const sshbuf,
    mut msghash: *mut u_char,
    mut msghashlen: size_t,
) -> libc::c_int {
    let mut r: libc::c_int = -(1 as libc::c_int);
    let mut chall: *mut sshbuf = 0 as *mut sshbuf;
...
unsafe extern "C" fn ssh_ecdsa_sk_verify(
    ...
    let mut original_signed: *mut sshbuf = 0 as *mut sshbuf;
    let mut webauthn_wrapper: *mut sshbuf = 0 as *mut sshbuf;
    let mut webauthn_exts: *mut sshbuf = 0 as *mut sshbuf;
...
static mut sshkey_ecdsa_sk_funcs: sshkey_impl_funcs = unsafe {
    {
        let mut init = sshkey_impl_funcs {
            size: None,
            alloc: None,
            cleanup: Some(ssh_ecdsa_sk_cleanup as unsafe extern "C" fn(*mut sshkey) -> ()),
            equal: Some(
                ssh_ecdsa_sk_equal
