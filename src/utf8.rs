...
unsafe extern "C" fn grow_dst(
    ...
    mut sz: *mut size_t,
    mut maxsz: size_t,
    mut dp: *mut *mut libc::c_char,
    mut need: size_t,
) -> libc::c_int {
    let mut tp: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut tsz: size_t = 0;
    if (*dp).offset(need as isize) < (*dst).offset(*sz as isize) {
        return 0 as libc::c_int;
    }
    tsz = (*sz).wrapping_add(128 as libc::c_int as libc::c_ulong);
    if tsz > maxsz {
        tsz = maxsz;
    }
    tp = recallocarray(
    ...
    *dp = tp.offset((*dp).offset_from(*dst) as libc::c_long as isize);
    *dst = tp;
    *sz = tsz;
    return 0 as libc::c_int;
}
pub unsafe extern "C" fn vasnmprintf(
    ...
    let mut total_width: libc::c_int = 0;
    ...
    if !(ret <= 0 as libc::c_int) {
        ...
        if dst.is_null() {
            ...
            if maxsz > 2147483647 as libc::c_int as libc::c_ulong {
                maxsz = 2147483647 as libc::c_int as size_t;
            }
            sp = src;
            dp = dst;
            ret = 0 as libc::c_int;
            print = 1 as libc::c_int;
            total_width = 0 as libc::c_int;
            max_width = if wp.is_null() {
                2147483647 as libc::c_int
            } else {
                *wp
            };
            while *sp as libc::c_int != '\0' as i32 {
                len = mbtowc(&mut wc, sp, __ctype_get_mb_cur_max());
                if len == -(1 as libc::c_int) {
                    ...
                    && (wc == '\n' as i32 || wc == '\r' as i32 || wc == '\t' as i32)
                {
                    width = 0 as libc::c_int;
                } else {
                    width = wcwidth(wc);
                    if width == -(1 as libc::c_int) && dangerous_locale() != 0 {
                        ret = -(1 as libc::c_int);
                        break;
                    }
                }
                if width >= 0 as libc::c_int {
                    if print != 0
                        && (dp.offset_from(dst) as libc::c_long
                            >= (maxsz as libc::c_int - len) as libc::c_long
                            || total_width > max_width - width)
                    {
                        print = 0 as libc::c_int;
                    }
                    if print != 0 {
                        if grow_dst(&mut dst, &mut sz, maxsz, &mut dp, len as size_t)
                            == -(1 as libc::c_int)
                        {
                            ret = -(1 as libc::c_int);
                            break;
                    ...
                    while len > 0 as libc::c_int {
                        if print != 0
                            && (dp.offset_from(dst) as libc::c_long
                                >= (maxsz as libc::c_int - 4 as libc::c_int) as libc::c_long
                                || total_width > max_width - 4 as libc::c_int)
                        {
                            print = 0 as libc::c_int;
                        }
                        if print != 0 {
                            ...
                            ) == -(1 as libc::c_int)
                            {
                                ...
                                width = tp.offset_from(dp) as libc::c_long as libc::c_int;
                                total_width += width;
                                dp = tp;
                            }
                        } else {
            ...
            *str = dst;
            ...
            if ret < maxsz as libc::c_int && print == 0 {
                ret = -(1 as libc::c_int);
            }
            return ret;
        }
...
pub unsafe extern "C" fn snmprintf(
...
pub unsafe extern "C" fn asmprintf(
    ...
    let mut ap: ::core::ffi::VaListImpl;
    let mut ret: libc::c_int = 0;
    *outp = 0 as *mut libc::c_char;
    ap = args.clone();
    ret = vasnmprintf(outp, sz, wp, fmt, ap.as_va_list());
    return ret;
}
...
pub unsafe extern "C" fn fmprintf(
    ...
    let mut ret: libc::c_int = 0;
    ap = args.clone();
    ret = vfmprintf(stream, fmt, ap.as_va_list());
    return ret;
}
pub unsafe extern "C" fn mprintf(mut fmt: *const libc::c_char, mut args: ...) -> libc::c_int {
    let mut ap: ::core::ffi::VaListImpl;
    let mut ret: libc::c_int = 0;
    ap = args.clone();
    ret = vfmprintf(stdout, fmt, ap.as_va_list());
    return ret;
}
pub unsafe extern "C" fn msetlocale() {
    let mut vars: [*const libc::c_char; 4] = [
        b"LC_ALL\0" as *const u8 as *const libc::c_char,
        b"LC_CTYPE\0" as *const u8 as *const libc::c_char,
    ...
    i = 0 as libc::c_int;
    while !(vars[i as usize]).is_null() {
        cp = getenv(vars[i as usize]);
        if cp.is_null() {
            i += 1;
            i;
        } else {
            ...
            if (!(strcasestr(cp, b"UTF-8\0" as *const u8 as *const libc::c_char)).is_null()
                || !(strcasestr(cp, b"UTF8\0" as *const u8 as *const libc::c_char)).is_null())
                && (!(setlocale(
                    0 as libc::c_int,
                ...
                .is_null()
                    || !(setlocale(
                        0 as libc::c_int,
                        b"POSIX.UTF-8\0" as *const u8 as *const libc::c_char,
                    ))
                    .is_null())
            {
                return;
