...
pub unsafe extern "C" fn sshkey_serialize_sk(
    ...
    r = crate::sshbuf_getput_basic::sshbuf_put_cstring(b, (*key).sk_application);
...
unsafe extern "C" fn to_blob_buf(
    ...
    ret = crate::sshbuf_getput_basic::sshbuf_put_cstring(b, typename);
    if ret != 0 as libc::c_int {
        return ret;
    }
...
unsafe extern "C" fn private2_check_padding(
    ...
    loop {
        ...
        r = crate::sshbuf_getput_basic::sshbuf_get_u8(decrypted, &mut pad);
        if r != 0 as libc::c_int {
...
unsafe extern "C" fn cert_parse(
    ...
    ret = crate::sshbuf_getput_basic::sshbuf_get_u64(b, &mut (*(*key).cert).serial);
    if ret != 0 as libc::c_int
        || {
            ret = crate::sshbuf_getput_basic::sshbuf_get_u32(b, &mut (*(*key).cert).type_0);
            ret != 0 as libc::c_int
        }
        || {
        ...
        || {
            ret = crate::sshbuf_getput_basic::sshbuf_get_u64(b, &mut (*(*key).cert).valid_after);
            ret != 0 as libc::c_int
        }
        || {
            ret = crate::sshbuf_getput_basic::sshbuf_get_u64(b, &mut (*(*key).cert).valid_before);
            ret != 0 as libc::c_int
        }
        || {
...
pub unsafe extern "C" fn sshkey_certify_custom(
    ...
    ret = crate::sshbuf_getput_basic::sshbuf_put_cstring(cert, sshkey_ssh_name(k));
    if !(ret != 0 as libc::c_int) {
        arc4random_buf(
        ...
        if !(ret != 0 as libc::c_int) {
            ...
            if !(ret != 0 as libc::c_int) {
                ret = crate::sshbuf_getput_basic::sshbuf_put_u64(cert, (*(*k).cert).serial);
                if !(ret != 0 as libc::c_int
                    || {
                        ret = crate::sshbuf_getput_basic::sshbuf_put_u32(cert, (*(*k).cert).type_0);
                        ret != 0 as libc::c_int
                    }
                    || {
                    ...
                    if principals.is_null() {
                        ...
                        match current_block {
                            ...
                            _ => {
                                ...
                                if !(ret != 0 as libc::c_int
                                    || {
                                        ret = crate::sshbuf_getput_basic::sshbuf_put_u64(
                                            cert,
                                            (*(*k).cert).valid_after,
                                        );
                                        ret != 0 as libc::c_int
...
pub unsafe extern "C" fn sshkey_private_serialize_opt(
    ...
    r = crate::sshbuf_getput_basic::sshbuf_put_cstring(b, sshkey_ssh_name(key));
    if !(r != 0 as libc::c_int) {
        if sshkey_is_cert(key) != 0 {
            if ((*key).cert).is_null()
                ...
                match current_block {
                    ...
                    _ => {
                        ...
                        if !(r != 0 as libc::c_int) {
                            ...
                            {
                                check = arc4random();
                                r = crate::sshbuf_getput_basic::sshbuf_put_u32(encrypted, check);
                                if !(r != 0 as libc::c_int || {
                                    r = crate::sshbuf_getput_basic::sshbuf_put_u32(
                                        encrypted, check,
                                    );
    ...
    *pubkeyp = 0 as *mut sshkey;
    ...
    if decrypted.is_null() {
        ...
        if !(r != 0 as libc::c_int
            ...
            || {
                r = crate::sshbuf_getput_basic::sshbuf_get_u32(decoded, &mut nkeys);
                r != 0 as libc::c_int
            })
        {
