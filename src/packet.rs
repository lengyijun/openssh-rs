...
unsafe extern "C" fn compress_buffer(
    ...
    (*(*ssh).state).compression_out_stream.next_in = crate::sshbuf::sshbuf_mutable_ptr(in_0);
...
unsafe extern "C" fn uncompress_buffer(
    ...
    (*(*ssh).state).compression_in_stream.next_in = crate::sshbuf::sshbuf_mutable_ptr(in_0);
...
pub unsafe extern "C" fn ssh_packet_send2_wrapped(mut ssh: *mut ssh) -> libc::c_int {
    ...
    if !comp.is_null() && (*comp).enabled != 0 {
        ...
        r = crate::sshbuf::sshbuf_consume((*state).outgoing_packet, 5 as libc::c_int as size_t);
        if r != 0 as libc::c_int {
    ...
    match current_block {
        6057473163062296781 => {
            ...
            if !(r != 0 as libc::c_int) {
                ...
                len = crate::sshbuf::sshbuf_len((*state).outgoing_packet) as u_int;
                cp = crate::sshbuf::sshbuf_mutable_ptr((*state).outgoing_packet);
                if cp.is_null() {
                    r = -(1 as libc::c_int);
                } else {
...
unsafe extern "C" fn ssh_packet_read_poll2_mux(
    ...
    *typep = 0 as libc::c_int as u_char;
    ...
    if r != 0 as libc::c_int
        || {
            r = crate::sshbuf::sshbuf_consume((*state).input, need);
            r != 0 as libc::c_int
        }
        || {
    ...
    if aadlen != 0 && (*state).packlen == 0 as libc::c_int as libc::c_uint {
        ...
        if r != 0 as libc::c_int {
            ...
            if r != 0 as libc::c_int {
                ...
                r = crate::sshbuf::sshbuf_consume((*state).input, block_size as size_t);
                if r != 0 as libc::c_int {
                    current_block = 11636842938881510489;
                } else {
                    current_block = 17281240262373992796;
...
pub unsafe extern "C" fn ssh_packet_process_read(
    ...
    if (*state).packet_discard != 0 {
        r = crate::sshbuf::sshbuf_consume_end((*state).input, rlen);
        if r != 0 as libc::c_int {
            return r;
        }
        (*state).keep_alive_timeouts = 0 as libc::c_int;
...
pub unsafe extern "C" fn ssh_packet_write_poll(mut ssh: *mut ssh) -> libc::c_int {
    ...
    if len > 0 as libc::c_int {
        ...
        r = crate::sshbuf::sshbuf_consume((*state).output, len as size_t);
        if r != 0 as libc::c_int {
            return r;
        }
    }
...
unsafe extern "C" fn newkeys_to_blob(
    ...
    if !(r != 0 as libc::c_int
        ...
        || {
            r = crate::sshbuf_getput_basic::sshbuf_put_string(
                b,
                (*enc).iv as *const libc::c_void,
                (*enc).iv_len as size_t,
            );
            r != 0 as libc::c_int
        })
    {
...
pub unsafe extern "C" fn sshpkt_put(
    ...
    return crate::sshbuf_getput_basic::sshbuf_put((*(*ssh).state).outgoing_packet, v, len);
}
...
pub unsafe extern "C" fn sshpkt_put_string(
    ...
    return crate::sshbuf_getput_basic::sshbuf_put_string((*(*ssh).state).outgoing_packet, v, len);
}
...
pub unsafe extern "C" fn sshpkt_get_string(
    ...
    return crate::sshbuf_getput_basic::sshbuf_get_string(
...
pub unsafe extern "C" fn sshpkt_get_cstring(
    ...
    return crate::sshbuf_getput_basic::sshbuf_get_cstring(
        (*(*ssh).state).incoming_packet,
        valp,
        lenp,
    );
