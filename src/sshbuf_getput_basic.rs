...
unsafe extern "C" fn check_offset(
    ...
    if (crate::sshbuf::sshbuf_ptr(buf)).is_null() {
        return -(1 as libc::c_int);
    }
    if offset >= (18446744073709551615 as libc::c_ulong).wrapping_sub(len) {
        return -(10 as libc::c_int);
    }
    if offset.wrapping_add(len) > crate::sshbuf::sshbuf_len(buf) {
        return if wr != 0 {
            -(9 as libc::c_int)
        } else {
            -(3 as libc::c_int)
...
unsafe extern "C" fn check_roffset(
    ...
    *p = (crate::sshbuf::sshbuf_ptr(buf)).offset(offset as isize);
    return 0 as libc::c_int;
}
...
pub unsafe extern "C" fn sshbuf_peek_string_direct(
    ...
    if crate::sshbuf::sshbuf_len(buf) < 4 as libc::c_int as libc::c_ulong {
    ...
    if (crate::sshbuf::sshbuf_len(buf)).wrapping_sub(4 as libc::c_int as libc::c_ulong)
        < len as libc::c_ulong
    {
        return -(3 as libc::c_int);
    }
