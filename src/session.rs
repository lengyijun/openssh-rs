...
unsafe extern "C" fn session_pty_req(mut ssh: *mut ssh, mut s: *mut Session) -> libc::c_int {
    ...
    r = sshpkt_get_cstring(ssh, &mut (*s).term, 0 as *mut size_t);
    if r != 0 as libc::c_int
        || {
            r = sshpkt_get_u32(ssh, &mut (*s).col);
            r != 0 as libc::c_int
...
unsafe extern "C" fn session_subsystem_req(mut ssh: *mut ssh, mut s: *mut Session) -> libc::c_int {
    ...
    let mut i: u_int = 0;
    r = sshpkt_get_cstring(ssh, &mut (*s).subsys, 0 as *mut size_t);
    if r != 0 as libc::c_int || {
        r = sshpkt_get_end(ssh);
        r != 0 as libc::c_int
    } {
...
unsafe extern "C" fn session_x11_req(mut ssh: *mut ssh, mut s: *mut Session) -> libc::c_int {
    ...
    if r != 0 as libc::c_int
        || {
            r = sshpkt_get_cstring(ssh, &mut (*s).auth_proto, 0 as *mut size_t);
            r != 0 as libc::c_int
        }
        || {
            r = sshpkt_get_cstring(ssh, &mut (*s).auth_data, 0 as *mut size_t);
            r != 0 as libc::c_int
        }
        || {
...
unsafe extern "C" fn session_exec_req(mut ssh: *mut ssh, mut s: *mut Session) -> libc::c_int {
    ...
    r = sshpkt_get_cstring(ssh, &mut command, 0 as *mut size_t);
    if r != 0 as libc::c_int || {
        r = sshpkt_get_end(ssh);
        r != 0 as libc::c_int
    } {
...
unsafe extern "C" fn session_env_req(mut ssh: *mut ssh, mut s: *mut Session) -> libc::c_int {
    ...
    if r != 0 as libc::c_int
        || {
            r = sshpkt_get_cstring(ssh, &mut val, 0 as *mut size_t);
            r != 0 as libc::c_int
        }
        || {
            r = sshpkt_get_end(ssh);
...
unsafe extern "C" fn session_signal_req(mut ssh: *mut ssh, mut s: *mut Session) -> libc::c_int {
    ...
    let mut success: libc::c_int = 0 as libc::c_int;
    r = sshpkt_get_cstring(ssh, &mut signame, 0 as *mut size_t);
    if r != 0 as libc::c_int || {
        r = sshpkt_get_end(ssh);
        r != 0 as libc::c_int
    } {
