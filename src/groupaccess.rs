...
static mut groups_byname: *mut *mut libc::c_char =
    0 as *const *mut libc::c_char as *mut *mut libc::c_char;
pub unsafe extern "C" fn ga_init(mut user: *const libc::c_char, mut base: gid_t) -> libc::c_int {
    let mut groups_bygid: *mut gid_t = 0 as *mut gid_t;
    let mut i: libc::c_int = 0;
    let mut j: libc::c_int = 0;
    let mut retry: libc::c_int = 0 as libc::c_int;
    let mut gr: *mut group = 0 as *mut group;
    if ngroups > 0 as libc::c_int {
        ga_free();
    }
    ngroups = 65536 as libc::c_int;
    ngroups = (if 65536 as libc::c_int as libc::c_long > sysconf(_SC_NGROUPS_MAX as libc::c_int) {
    ...
    while getgrouplist(user, base, groups_bygid, &mut ngroups) == -(1 as libc::c_int) {
        let fresh0 = retry;
        retry = retry + 1;
        if fresh0 > 0 as libc::c_int {
            sshfatal(
                b"groupaccess.c\0" as *const u8 as *const libc::c_char,
                (*::core::mem::transmute::<&[u8; 8], &[libc::c_char; 8]>(b"ga_init\0")).as_ptr(),
                67 as libc::c_int,
                0 as libc::c_int,
                SYSLOG_LEVEL_FATAL,
                0 as *const libc::c_char,
                b"getgrouplist: groups list too small\0" as *const u8 as *const libc::c_char,
            );
        }
        groups_bygid = xreallocarray(
            groups_bygid as *mut libc::c_void,
            ngroups as size_t,
            ::core::mem::size_of::<gid_t>() as libc::c_ulong,
        ) as *mut gid_t;
    }
    ...
    j = 0 as libc::c_int;
    while i < ngroups {
        gr = getgrgid(*groups_bygid.offset(i as isize));
        if !gr.is_null() {
            let fresh1 = j;
            j = j + 1;
...
pub unsafe extern "C" fn ga_match(
    mut groups: *const *mut libc::c_char,
    mut n: libc::c_int,
) -> libc::c_int {
    let mut i: libc::c_int = 0;
    let mut j: libc::c_int = 0;
    ...
    while i < ngroups {
        ...
        while j < n {
            if match_pattern(
                *groups_byname.offset(i as isize),
                *groups.offset(j as isize),
            ) != 0
            {
...
pub unsafe extern "C" fn ga_match_pattern_list(
    mut group_pattern: *const libc::c_char,
) -> libc::c_int {
    let mut i: libc::c_int = 0;
    let mut found: libc::c_int = 0 as libc::c_int;
    i = 0 as libc::c_int;
    while i < ngroups {
        match match_usergroup_pattern_list(*groups_byname.offset(i as isize), group_pattern) {
...
pub unsafe extern "C" fn ga_free() {
    let mut i: libc::c_int = 0;
    if ngroups > 0 as libc::c_int {
        i = 0 as libc::c_int;
        while i < ngroups {
